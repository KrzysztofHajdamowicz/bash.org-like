...
class QuoteModelTest(TestCase):
    def test_default_status_is_pending(self):
        ...
        self.assertEqual(quote.status, 1)
...
class QuoteWorkflowTest(TestCase):
    ...
    def test_approve_quote(self):
        ...
        response = self.client.get(reverse("quote_accept", args=[quote.id]), HTTP_REFERER="/manage/")
        self.assertEqual(response.status_code, 302)

        quote.refresh_from_db()
        self.assertEqual(quote.status, 3)
    ...
    def test_reject_quote(self):
        ...
        response = self.client.get(reverse("quote_reject", args=[quote.id]), HTTP_REFERER="/manage/")
        self.assertEqual(response.status_code, 302)

        quote.refresh_from_db()
        self.assertEqual(quote.status, 2)
        self.assertEqual(quote.acceptant, self.admin)
    ...
    def test_upvote_approved_quote(self):
        ...
        for _ in range(3):
            self.client.get(reverse("quote_vote_up", args=[quote.id]), HTTP_REFERER="/")
    ...
    def test_downvote_approved_quote(self):
        ...
        for _ in range(2):
            self.client.get(reverse("quote_vote_down", args=[quote.id]), HTTP_REFERER="/")
    ...
    def test_delete_quote(self):
        ...
        self.client.get(reverse("quote_delete", args=[quote_id]), HTTP_REFERER="/manage/")
    ...
    def test_accept_requires_login(self):
        response = self.client.get(reverse("quote_accept", args=[self.quotes[0].id]))
        self.assertEqual(response.status_code, 302)
        self.assertIn("/login/", response.url)
...
class QuoteDetailViewTest(TestCase):
    def test_view_single_quote(self):
        quote = Quote.objects.create(content="<a> test quote", status=3)
